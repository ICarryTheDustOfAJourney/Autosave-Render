...
- enter a short comment to be appended to the new directory's name. Free shorthand reminder like "evee only", "applied" or "!" when used in production etc. Use only characters allowed in your filesystem (Windows: no ":&lt;&gt;" etc). Intended to be useful when revisiting after a while
- render (&lt;F12>)
